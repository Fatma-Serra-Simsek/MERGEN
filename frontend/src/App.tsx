...
const App: React.FC = () => {
  ...
  let planKey = '';
  let plan: DayPlan = { todos: Array(8).fill({ text: '', done: false }), shopping: Array(8).fill(''), note: '' };
  if (selectedDay) {
    planKey = getPlanKey(year, month, selectedDay);
    plan = plans[planKey] || { todos: Array(8).fill({ text: '', done: false }), shopping: Array(8).fill(''), note: '' };
  }
  ...
  return (
    <div style={{ minHeight: '100vh', minWidth: '100vw', background: palette.light, margin: 0, padding: 0, boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
      {/* Üstte ay/yıl seçici */}
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gridAutoRows: 'minmax(90px, 1fr)', gap: 18, maxWidth: 1100, width: '100%', margin: '0 auto', marginBottom: 24 }}>
        {calendarDays.map((day, i) => (
          <div key={i} style={{
            background: day === null ? 'transparent' : [palette.lilac, palette.pink, palette.beige, palette.light, palette.dark][i % 5],
            borderRadius: 24,
            minHeight: 90,
            minWidth: 60,
      ...
      {modalOpen && (
        <div style={{
          ...
          background: 'rgba(77,81,109,0.18)',
          zIndex: 1000,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          ...
          <div style={{ background: palette.light, borderRadius: 18, padding: 24, minWidth: 400, maxWidth: 500, width: '90%', boxShadow: `0 8px 32px ${palette.lilac}99`, position: 'relative', display: 'flex', flexDirection: 'column', gap: 12 }} onClick={e => e.stopPropagation()}>
            ...
            {/* Yalnızca içerik kutuları */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {/* YAPILACAKLAR */}
              <div style={{ border: `1px solid ${palette.lilac}`, borderRadius: 8, background: palette.beige, padding: 8, marginBottom: 4 }}>
                <div style={{ color: palette.pink, fontWeight: 700, fontSize: 18, marginBottom: 4, textAlign: 'center' }}>YAPILACAKLAR</div>
                {plan.todos.map((todo, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    <input type="checkbox" checked={todo.done} onChange={e => {
                      const newTodos = plan.todos.map((t, idx) => idx === i ? { ...t, done: e.target.checked } : t);
                      updatePlan({ ...plan, todos: newTodos });
                    }} style={{ accentColor: palette.pink, marginLeft: 4 }} />
                    <input type="text" value={todo.text} onChange={e => {
                      const newTodos = plan.todos.map((t, idx) => idx === i ? { ...t, text: e.target.value } : t);
                      updatePlan({ ...plan, todos: newTodos });
                    }} placeholder="Görev..." style={{ flex: 1, border: 'none', borderBottom: `1px solid ${palette.lilac}` , background: 'transparent', fontSize: 15, color: palette.dark }} />
                  </div>
                ))}
              </div>
              {/* ALIŞVERİŞ LİSTESİ */}
              <div style={{ border: `1px solid ${palette.lilac}` , borderRadius: 8, background: palette.beige, padding: 8, marginBottom: 4 }}>
                <div style={{ color: palette.pink, fontWeight: 700, fontSize: 18, marginBottom: 4, textAlign: 'center' }}>ALIŞVERİŞ LİSTESİ</div>
                {plan.shopping.map((item, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    ...
                    }} placeholder="..." style={{ flex: 1, border: 'none', borderBottom: `1px solid ${palette.lilac}` , background: 'transparent', fontSize: 15, color: palette.dark }} />
                  </div>
                ))}
              </div>
              {/* NOT */}
              <div style={{ border: `1px solid ${palette.lilac}` , borderRadius: 8, background: palette.beige, padding: 8, marginBottom: 4 }}>
                <div style={{ color: palette.pink, fontWeight: 700, fontSize: 18, marginBottom: 4, textAlign: 'center' }}>NOT</div>
                <textarea value={plan.note} onChange={e => updatePlan({ ...plan, note: e.target.value })} style={{ width: '100%', minHeight: 40, border: 'none', background: 'transparent', fontSize: 15, resize: 'vertical', borderBottom: `1px solid ${palette.lilac}` , color: palette.dark }} placeholder="Not..." />
              </div>
            </div>
          </div>
        </div>
